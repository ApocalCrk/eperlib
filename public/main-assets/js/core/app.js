...
(function (window, document, $) {
  ...
  const fetchData = debounce(function (value) {
    $.getJSON('/user/search/', { value: value }, function (data) {
      $('ul.search-list').empty(); 
      let $pageList = '<li class="d-flex align-items-center">' +
        '<a href="#">' +
        '<h6 class="section-label mt-75 mb-0">Pages</h6>' +
        '</a>' +
        '</li>'; 

      let $booksList = '<li class="d-flex align-items-center">' +
        '<a href="#">' +
        '<h6 class="section-label mt-75 mb-0">Buku</h6>' +
        '</a>' +
        '</li>'; 
      ...
      for (let i = 0; i < data.listItems.length; i++) {
        if (data.listItems[i].name.toLowerCase().indexOf(value) > -1) {
          if (data.listItems[i].type === 'pages') {
            $pageList +=
              '<li class="auto-suggestion d-flex align-items-center">' +
              '<a class="d-flex align-items-center justify-content-between w-100" href=' +
              ...
              '</li>';
            a++;
          } else if (data.listItems[i].type === 'books') {
            $booksList +=
              '<li class="auto-suggestion">' +
              '<a class="d-flex align-items-center justify-content-between w-100" href="' + data.listItems[i].url + '">' +
              '<div class="d-flex">' +
              '<div class="me-75">' +
              '<img src="' + window.location.origin + '/' + data.listItems[i].img + '" alt="png" height="32" />' +
              '</div>' +
              '<div class="search-data">' +
              '<p class="search-data-title mb-0">' + data.listItems[i].name + '</p>' +
              '<small class="text-muted">' + data.listItems[i].author + '</small>' +
              '</div>' +
              '</div>' +
              '</a>' +
              '</li>';
      ...
      if (a === 0) {
        $pageList +=
          ...
          '<h6 class="section-label mt-75 mb-0">Tidak ada halaman ditemukan</h6>' +
          '</a>' +
          '</li>';
      }
      ...
      if (b === 0) {
        $booksList +=
          '<li class="d-flex align-items-center justify-content-center mb-1">' +
          '<a href="javascript:void(0)">' +
          '<h6 class="section-label mt-75 mb-0">Tidak ada buku ditemukan</h6>' +
          '</a>' +
          '</li>';
      }

      const $htmlList = $pageList + $booksList;
      $('ul.search-list').append($htmlList);
    });
  }, 300); 
  ...
  searchInputInputfield.on('keyup', function (e) {
    ...
    if (e.keyCode !== 38 && e.keyCode !== 40 && e.keyCode !== 13) {
      ...
      if (value !== '') {
        ...
        fetchData(value); // Trigger fetching data using debounce
      }
    }
  });
  ...
  if (localStorage.getItem(dataLayout + '-current-skin')) {
    setLayout(localStorage.getItem(dataLayout + '-current-skin'));
  } else {
